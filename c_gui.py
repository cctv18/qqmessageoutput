...
def deal():
    db, k, od, md, fr, tr, db2 = dbfile.get(), key.get(
    ), outdir.get(), mode.get(), qq.get(), troop.get(), dbfile2.get()
    if (db == '' or od == ''):
        info.set('!请确认填写完整')
        return
    if (md == 'friend' and fr == ''):
...
ttk.Label(window, text="github.com/ctem049/qqmessageoutput ver{}".format(version)
...
ttk.Button(window, text="选择", command=SelectPath,
...
ttk.Button(window, text="选择", command=SelectOutdir,
...
# 4.1 导出全部
ttk.Radiobutton(window, text='导出所有记录', variable=mode, value='all',
                command=modeCheck).grid(row=4, column=0, sticky="e")

# 4.2 导出指定QQ号
ttk.Radiobutton(window, text='导出指定好友', variable=mode, value='friend',
                command=modeCheck).grid(row=5, column=0, sticky="e")
qq = tk.StringVar()
e4 = ttk.Entry(window, textvariable=qq, state='disabled')
e4.grid(row=5, column=1)

# 4.3 导出指定QQ群
ttk.Radiobutton(window, text='导出指定群聊', variable=mode, value='troop',
                command=modeCheck).grid(row=6, column=0, sticky="e")
troop = tk.StringVar()
...
ttk.Label(window, text="同时导出slowtable: ").grid(row=7, column=0, sticky="e")
...
ttk.Button(window, text="选择", command=SelectDbfile2,
           width=5).grid(row=7, column=3)
